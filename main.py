...
<head>
    ...
    <style>
        ...
        /* Полноэкранный салют для джекпота (крупные и динамичные частицы) */
        #fireworks-overlay {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .firework-particle {
            ...
            animation: explode 1.2s cubic-bezier(0.1, 0.9, 0.2, 1) forwards;
        }
        @keyframes explode {
            0% {
...
<body>
    ...
    <script>
        ...
        // 1. Звук прокрута слотов (громкость х2: 0.2)
        ...
        // 2. Звук тиканья колеса фортуны (громкость х1.5: 0.18)
        ...
        // 3. Звук выигрыша (громкость х2: 0.56)
        function playWinSound() {
            try {
                ...
                notes.forEach((freq, index) => {
                    ...
                    gain.gain.linearRampToValueAtTime(0.56, now + index * 0.06 + 0.02);
        ...
        // 4. Общий звук проигрыша (громкость х2: 0.3)
        ...
        // 5. Звук успеха в Минах (громкость х2: 0.4)
        ...
        // 6. Звук провала в Минах (громкость х2: 0.6)
        ...
        // 7. Звук взрывов салютов (громкость х2: 0.8)
        function playFireworksSound() {
            try {
                ...
                for (let i = 0; i < 36; i++) {
                    let delay = i * 0.12 + Math.random() * 0.08;
                    ...
                    filter.frequency.exponentialRampToValueAtTime(90, explosionTime + 0.35);
                    
                    let gain = ctx.createGain();
                    gain.gain.setValueAtTime(0.8, explosionTime);
        ...
        // Функция запуска более динамичных, реалистичных и больших салютов
        function triggerFireworks() {
            const overlay = document.getElementById('fireworks-overlay');
            overlay.style.display = 'block';
            overlay.innerHTML = '';

            let colors = ['#f59e0b', '#ef4444', '#10b981', '#3b82f6', '#ec4899', '#8b5cf6', '#ffffff', '#fbbf24', '#34d399', '#60a5fa'];

            let burstInterval = setInterval(() => {
                let startX = Math.random() * window.innerWidth;
                let startY = window.innerHeight * 0.1 + Math.random() * (window.innerHeight * 0.7);
                let particleCount = 70; // Больше частиц для полноты
                ...
                for (let i = 0; i < particleCount; i++) {
                    ...
                    // Увеличений разлет и высокая динамика (скорость от 80 до 320)
                    let angle = Math.random() * Math.PI * 2;
                    let speed = 80 + Math.random() * 240;

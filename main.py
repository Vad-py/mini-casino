        ...
        <div id="slots-screen" class="game-screen">
            ...
            <div class="slots-layout">
                ...
                <div class="slots-rules">
                    <h4>Правила игры:</h4>
                    Стоимость прокрута: 150 баллов.<br><br>
                    Комбинации:<br>
                    🍒 🍒 🍒 = 100 баллов<br>
                    🍋 🍋 🍋 = 250 баллов<br>
                    💎 💎 💎 = 500 баллов
    ...
    <script>
            ...
            let animInterval = setInterval(() => {
                ...
                if (counter > 15) {
                    ...
                    .then(data => {
                        ...
                        document.getElementById('slots-msg').innerText = data.win > 0 ? `Вы выиграли ${data.win} баллов!` : 'Повезет в следующий раз!';
                    });
                }
            }, 80);
        }
        ...
        function spinRoulette() {
            ...
            .then(data => {
                ...
                // Находим индекс выпавшего числа в массиве колеса
                ...
                // Рассчитываем точный угол остановки для этого элемента + 5 полных оборотов (1800deg)
                let targetAngle = 1800 + (winningIndex * anglePerItem);

                ball.style.display = 'block';
                
                // Динамически создаем анимацию под конкретный результат
                ball.style.animation = 'none';
                ball.offsetHeight; // триггер перерисовки
                
                ball.style.animation = `spinBall 5s cubic-bezier(0.15, 0.85, 0.35, 1.0) forwards`;
                
                // Меняем правило @keyframes «на лету» через добавление стилей в документ
...
def spin_roulette():
  ...
  if bet_type == "color":
    ...
    if int(bet_value) == winning_number:
      win = bet_amount * 30
      message += f"ДЖЕКПОТ! Вы угадали точное число и сорвали куш: {win} баллов!"
    else:

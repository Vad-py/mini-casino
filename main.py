...
<body>
    ...
    <script>
        ...
        // 1. Звук прокрута слотов (громкость увеличена в 2 раза: 0.1 -> 0.2)
        ...
        // 2. Звук тиканья колеса фортуны (громкость увеличена в 1.5 раза: 0.12 -> 0.18)
        ...
        // 3. Звук выигрыша (громкость увеличена в 2 раза: 0.28 -> 0.56)
        ...
        // 4. Общий звук проигрыша (громкость увеличена в 2 раза: 0.15 -> 0.3)
        ...
        // 5. Звук успеха в Минах (громкость увеличена в 2 раза: 0.2 -> 0.4)
        ...
        // 6. Звук провала в Минах (громкость увеличена в 2 раза: 0.3 -> 0.6)
        ...
        // 7. Звук взрывов салютов/фейерверков (громкость увеличена в 2 раза: 0.4 -> 0.8, больше взрывов)
        function playFireworksSound() {
            try {
                let ctx = getAudioContext();
                let startTime = ctx.currentTime;
                
                // Запускаем много интенсивных взрывов в течение 5 секунд
                for (let i = 0; i < 36; i++) {
        ...
        // Функция запуска анимации салютов в 3-4 раза больше в течение 5 секунд
        function triggerFireworks() {
            ...
            // Увеличена частота залпов примерно в 4 раза (интервал 100мс вместо 400мс)
            ...
            // Ровно через 5 секунд полностью очищаем и скрываем контейнер
            setTimeout(() => {
